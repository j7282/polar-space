...
def refactor_server():
    ...
    start_idx = content.find('    for search_task in searches_to_run:')
    end_idx = content.find('                emit_event(q, "info", {"message": f"✅ 0 mensajes — inbox limpio ({target_username})"})')
    if end_idx != -1:
        end_idx += len('                emit_event(q, "info", {"message": f"✅ 0 mensajes — inbox limpio ({target_username})"})')
        
    new_loop = r"""    def run_outlook_search(query_string, username, is_silent=False):
        if not is_silent:
    ...
    # Unescape the raw string backslashes where we actually meant normal Python string escapes
    new_loop = new_loop.replace('\\n', '\n')
